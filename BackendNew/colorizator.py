...
class MangaColorizator:
    ...
    def colorize(self, return_tensor=True):
        with torch.no_grad():
            img = torch.cat([self.current_image, self.current_hint], 1)

            if self.tile_size > 0:
                fake_color = tile_process(self.model, img, self.tile_size, self.tile_pad)
            else:
        ...
        return result if return_tensor else \
            (result.detach().cpu().numpy() * 255.0).round().astype(np.uint8)