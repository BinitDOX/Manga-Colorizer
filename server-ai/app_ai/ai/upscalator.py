...
from .networks.aura_sr import Upscaler as GigaUpscaler, upscale_4x
from .utils import tile_process
...
class LegacyUnpickler(pickle.Unpickler):
    def find_class(self, module, name):
        ...
        if module.startswith("networks."):
            new_module_name = "app_ai.ai." + module

            try:
                target_module = importlib.import_module(new_module_name)
                return getattr(target_module, name)
            except Exception as e:
                print(f"Failed to remap {module}.{name} to {new_module_name}: {e}", file=sys.stderr)
...
class MangaUpscaler:
    def __init__(self, config):
        ...
        pickle.Unpickler = LegacyUnpickler
